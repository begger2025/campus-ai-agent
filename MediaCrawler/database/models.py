...
class XhsNote(Base):
    ...
    publish_time_raw = Column(Text, comment='raw publish time text')
    publish_timestamp_ms = Column(BigInteger, index=True, comment='normalized publish timestamp ms')
    publish_date = Column(String(20), index=True, comment='normalized publish date')
    publish_year = Column(Integer, index=True, comment='publish year')
    publish_month = Column(Integer, index=True, comment='publish month')
    publish_day = Column(Integer, index=True, comment='publish day')
    first_crawled_at = Column(BigInteger, index=True, comment='first crawled timestamp')
    last_crawled_at = Column(BigInteger, index=True, comment='last crawled timestamp')
    crawl_count = Column(Integer, default=1, comment='crawl count')

class XhsCrawlHistory(Base):
    __tablename__ = 'xhs_crawl_history'
    id = Column(Integer, primary_key=True, comment='primary key')
    run_id = Column(String(255), unique=True, index=True, comment='crawl run id')
    platform = Column(String(32), default='xhs', comment='platform')
    source_keyword = Column(Text, default='', comment='source keyword')
    started_at = Column(BigInteger, index=True, comment='started at')
    finished_at = Column(BigInteger, index=True, comment='finished at')
    planned_detail_count = Column(Integer, default=0, comment='planned detail count')
    success_detail_count = Column(Integer, default=0, comment='success detail count')
    failed_detail_count = Column(Integer, default=0, comment='failed detail count')
    risk_control_triggered = Column(Integer, default=0, comment='risk control triggered')
    stop_reason = Column(Text, default='', comment='stop reason')
    failed_note_ids_json = Column(Text, default='[]', comment='failed note ids json')
    extra_json = Column(Text, default='{}', comment='extra json')
