...
class BaiduTieBaClient(AbstractApiClient):
    ...
    async def get_notes_by_keyword(
        ...
        try:
            ...
            utils.logger.info(
                "[BaiduTieBaClient.get_notes_by_keyword] "
                f"Current URL: {self.playwright_page.url}, "
                f"title: {await self.playwright_page.title()}, "
                f"s_post markers: {page_content.count('s_post')}"
            )
            ...
            if not notes:
                debug_dir = pathlib.Path(config.SAVE_DATA_PATH or "data") / "tieba" / "debug"
                debug_dir.mkdir(parents=True, exist_ok=True)
                safe_keyword = re.sub(r"[^\w\u4e00-\u9fff-]+", "_", keyword)[:40] or "keyword"
                debug_file = debug_dir / f"search_{safe_keyword}_page_{page}.html"
