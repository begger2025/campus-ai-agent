...
class TieBaExtractor:
    ...
    def extract_search_note_list(page_content: str) -> List[TiebaNote]:
        ...
        xpath_selector = "//div[contains(concat(' ', normalize-space(@class), ' '), ' s_post ')]"
        post_list = content_selector.xpath(xpath_selector)
        result: List[TiebaNote] = []
        seen_note_ids = set()
        for post in post_list:
            title_selector = post.xpath(".//span[contains(concat(' ', normalize-space(@class), ' '), ' p_title ')]/a")
            note_url = title_selector.xpath("./@href").get(default='').strip()
            note_id = title_selector.xpath("./@data-tid").get(default='').strip()
            if not note_id and note_url:
                note_id_match = re.search(r"/p/(\d+)", note_url)
                note_id = note_id_match.group(1) if note_id_match else ""
            if not note_id:
                continue

            if note_url and not note_url.startswith("http"):
                note_url = const.TIEBA_URL + note_url
            if note_id in seen_note_ids:
                continue
            seen_note_ids.add(note_id)

            tieba_link = post.xpath(".//a[contains(concat(' ', normalize-space(@class), ' '), ' p_forum ')]/@href").get(
                default='')
            if tieba_link and not tieba_link.startswith("http"):
                tieba_link = const.TIEBA_URL + tieba_link

            tieba_note = TiebaNote(note_id=note_id,
                                   title=''.join(title_selector.xpath(".//text()").getall()).strip(),
                                   desc=''.join(post.xpath(".//div[contains(concat(' ', normalize-space(@class), ' '), ' p_content ')]//text()").getall()).strip(),
                                   note_url=note_url,
                    ...
                    ".//a[starts-with(@href, '/home/main')]/@href").get(default=''),
                                   tieba_name=post.xpath(".//a[contains(concat(' ', normalize-space(@class), ' '), ' p_forum ')]//text()").get(default='').strip(),
                                   tieba_link=tieba_link,
                                   publish_time=post.xpath(".//font[contains(concat(' ', normalize-space(@class), ' '), ' p_date ')]/text()").get(
                                       default='').strip(), )
            result.append(tieba_note)

        new_card_xpath = (
            "//div[contains(concat(' ', normalize-space(@class), ' '), ' threadcardclass ') "
            "and .//a[contains(@href, '/p/')]]"
        )
        for card in content_selector.xpath(new_card_xpath):
            note_url = card.xpath(".//a[contains(@href, '/p/')]/@href").get(default='').strip()
            note_id_match = re.search(r"/p/(\d+)", note_url)
            note_id = note_id_match.group(1) if note_id_match else ""
            ...
            title = TieBaExtractor._join_text(
                card.xpath(".//div[contains(concat(' ', normalize-space(@class), ' '), ' title-wrap ')]//text()")
            )
            desc = TieBaExtractor._join_text(
                card.xpath(".//div[contains(concat(' ', normalize-space(@class), ' '), ' abstract-wrap ')]//text()")
            )
            if not title:
                title = TieBaExtractor._join_text(
                    card.xpath(".//div[contains(concat(' ', normalize-space(@class), ' '), ' title-content-wrap ')]//text()")
                )

            tieba_name = TieBaExtractor._join_text(
                card.xpath(".//div[contains(concat(' ', normalize-space(@class), ' '), ' forum-name ')]//text()")
            )
            if tieba_name.endswith("吧"):
                tieba_name = tieba_name[:-1].strip()

            tieba_link = card.xpath(
                ".//div[contains(concat(' ', normalize-space(@class), ' '), ' thread-forum-name ')]//a/@href"
            ).get(default='').strip()
            if tieba_link.startswith("//"):
                tieba_link = "https:" + tieba_link
            elif tieba_link.startswith("/"):
                tieba_link = const.TIEBA_URL + tieba_link

            top_title = TieBaExtractor._join_text(
                card.xpath(".//div[contains(concat(' ', normalize-space(@class), ' '), ' top-title ')]//text()")
            )
            publish_time_match = re.search(r"发布于\s*([0-9]{4}-[0-9]{1,2}-[0-9]{1,2})", top_title)
            publish_time = publish_time_match.group(1) if publish_time_match else ""

            user_link = card.xpath(
                ".//a[contains(concat(' ', normalize-space(@class), ' '), ' attention-wrap ')]/@href"
            ).get(default='').strip()
            if user_link.startswith("//"):
                user_link = "https:" + user_link
            elif user_link.startswith("/"):
                user_link = const.TIEBA_URL + user_link
            ...
            result.append(
                TiebaNote(
                    ...
                    desc=desc,
                    note_url=note_url,
                    user_nickname=TieBaExtractor._join_text(
                        card.xpath(".//a[contains(concat(' ', normalize-space(@class), ' '), ' attention-wrap ')]//text()")
                    ),
                    user_link=user_link,
    ...
    @staticmethod
    def _join_text(selector: Selector) -> str:
        return re.sub(r"\s+", " ", " ".join(text.strip() for text in selector.getall() if text.strip())).strip()
