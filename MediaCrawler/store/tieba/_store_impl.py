...
class TieBaDbStoreImplement(AbstractStore):
    async def store_content(self, content_item: Dict):
        ...
        try:
            ...
            # tieba 同 weibo，插入/更新共用一条路径、无独立 add_/update_ helper，
            # get_session() 已在退出时 rollback 并重新抛出，这里在外层兜住，跳过本条并告警。
            # 注意：约束能触发说明这一行已由竞态里的"赢家"写进库了——内容本身不丢；
            # 被丢弃的只是本次这个"输家"里那点更新的互动量。且默认 TIEBA_SKIP_EXISTING_NOTES=True
            # 时下一轮会跳过已入库帖子、不会重抓覆盖，所以别指望"下次重爬自然更新"。
            utils.logger.warning(
                f"[TieBaDbStoreImplement.store_content] note_id={note_id} 写入唯一键冲突（并发竞态），已跳过本次写入"
            )
