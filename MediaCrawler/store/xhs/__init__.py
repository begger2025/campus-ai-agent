...
from .xhs_note_assets import export_xhs_note_enhanced_json, normalize_xhs_image_entries
from .xhs_publish_time import extract_xhs_search_publish_time_text, resolve_xhs_note_publish_time
...
async def update_xhs_note(note_item: Dict):
    ...
    publish_fields = resolve_xhs_note_publish_time(note_item, crawl_ts_ms=utils.get_current_timestamp())
    note_item.update(publish_fields)
    ...
    local_db_item = {
        ...
        "publish_time_raw": publish_fields.get("publish_time_raw"),
        "publish_timestamp_ms": publish_fields.get("publish_timestamp_ms"),
        "publish_date": publish_fields.get("publish_date"),
        "publish_year": publish_fields.get("publish_year"),
        "publish_month": publish_fields.get("publish_month"),
        "publish_day": publish_fields.get("publish_day"),
    ...
    utils.logger.info(
        f"[store.xhs.update_xhs_note] publish time normalized, note_id={note_id}, "
        f"source={publish_fields.get('publish_time_source')}, raw={publish_fields.get('publish_time_raw')}, "
        f"publish_date={publish_fields.get('publish_date')}, "
        f"publish_timestamp_ms={publish_fields.get('publish_timestamp_ms')}"
    )
...
async def batch_get_existing_note_ids(note_ids: List[str]) -> Set[str]:
    ...
    if not callable(batch_getter):
        utils.logger.info(
            f"[store.xhs.batch_get_existing_note_ids] Current store backend does not support note existence lookup, "
    ...
    utils.logger.info(
        f"[store.xhs.batch_get_existing_note_ids] Checked {len(normalized_note_ids)} candidate note_ids, "
        f"existing in store: {len(existing_note_ids)}"
    )
    return existing_note_ids


async def create_xhs_crawl_history(history_item: Dict) -> None:
    store = XhsStoreFactory.create_store()
    creator = getattr(store, "create_crawl_history", None)
    if not callable(creator):
        utils.logger.info(
            f"[store.xhs.create_xhs_crawl_history] Current store backend does not support crawl history, "
            f"save option: {config.SAVE_DATA_OPTION}"
        )
        return

    await creator(history_item)


async def update_xhs_crawl_history(run_id: str, history_item: Dict) -> None:
    if not run_id:
        return

    store = XhsStoreFactory.create_store()
    updater = getattr(store, "update_crawl_history", None)
    if not callable(updater):
        utils.logger.info(
            f"[store.xhs.update_xhs_crawl_history] Current store backend does not support crawl history, "
            f"save option: {config.SAVE_DATA_OPTION}"
        )
        return

    await updater(run_id, history_item)


async def get_recent_notes(limit: int = 20, source_keyword: Optional[str] = None) -> List[Dict]:
    store = XhsStoreFactory.create_store()
    getter = getattr(store, "get_recent_notes", None)
    if not callable(getter):
        utils.logger.info(
            f"[store.xhs.get_recent_notes] Current store backend does not support recent note query, "
            f"save option: {config.SAVE_DATA_OPTION}"
        )
        return []

    return await getter(limit=limit, source_keyword=source_keyword)


async def get_recent_crawl_history(limit: int = 20, source_keyword: Optional[str] = None) -> List[Dict]:
    store = XhsStoreFactory.create_store()
    getter = getattr(store, "get_recent_crawl_history", None)
    if not callable(getter):
        utils.logger.info(
            f"[store.xhs.get_recent_crawl_history] Current store backend does not support crawl history query, "
            f"save option: {config.SAVE_DATA_OPTION}"
        )
        return []
...
async def update_xhs_note_image(note_id, pic_content, extension_file_name):
    ...
    return await XiaoHongShuImage().store_image(
...
def find_existing_xhs_note_image(note_id: str, file_index: int) -> str:
    return XiaoHongShuImage().find_existing_image(note_id, str(file_index))


async def export_xhs_note_enhanced(note_item: Dict, image_download_meta: Optional[List[Dict]] = None):
    note_item = dict(note_item)
    note_item.setdefault("source_keyword", source_keyword_var.get())
    return await export_xhs_note_enhanced_json(
        note_item,
