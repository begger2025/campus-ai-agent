...
class XiaoHongShuImage(AbstractStoreImage):
    def __init__(self):
        self.image_store_path = getattr(config, "XHS_MEDIA_OUTPUT_DIR", "output/xhs_media")

    def find_existing_image(self, notice_id: str, file_stem: str) -> str:
        notice_dir = pathlib.Path(self.image_store_path) / notice_id
        if not notice_dir.exists():
            return ""

        existing_files = sorted(notice_dir.glob(f"{file_stem}.*"))
        if not existing_files:
            return ""
        return existing_files[0].as_posix()
    ...
    async def save_image(self, notice_id: str, pic_content: str, extension_file_name):
        ...
        file_stem = pathlib.Path(extension_file_name).stem
        if getattr(config, "XHS_SKIP_EXISTING_MEDIA", True):
            existing_path = self.find_existing_image(notice_id, file_stem)
            if existing_path:
                utils.logger.info(
                    f"[XiaoHongShuImageStoreImplement.save_image] skip existing image {existing_path}"
                )
                return {"local_path": existing_path, "skipped": True}
        ...
        return {"local_path": pathlib.Path(save_file_name).as_posix(), "skipped": False}
