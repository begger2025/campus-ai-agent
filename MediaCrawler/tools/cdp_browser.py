...
class CDPBrowserManager:
    ...
    @staticmethod
    def _is_xhs_related_url(url: Optional[str]) -> bool:
        if not url:
            return False
        return "xiaohongshu.com" in url or "rednote.com" in url
    ...
    async def _get_browser_websocket_url(self, debug_port: int) -> str:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                utils.logger.info(f"[CDPBrowserManager] Fetching browser websocket URL from {version_url}")
    ...
    async def _create_browser_context(
        ...
        if contexts:
            ...
            for context in contexts:
                if any(self._is_xhs_related_url(page.url) for page in context.pages):
            ...
            if preferred_context is None:
                for context in contexts:
                    ...
                    if "web_session" in cookie_names or "a1" in cookie_names:
                        preferred_context = context
                        break

            if preferred_context is not None:
                browser_context = preferred_context
                utils.logger.info("[CDPBrowserManager] Using existing browser context with XHS pages/cookies")
            else:
