...
#   - 而 escalating **必须有正面证据**（还在扩大），不许作为"拿不准"的去处：紧急度只能被
#     证据抬高，不能被无知抬高。
...
- 确实存在一件待处置的事、但帖子里读不到结论时：判 ongoing（不要凭空替学校宣布它已经结束）。
  但**不许**因为"读不出来"就判 escalating——escalating 需要"还在扩大"的**正面证据**
  （新帖仍在增加、蔓延到更多平台、出现联名）。
...
字段要求：
- lifecycle：只能是 "not_applicable" / "resolved" / "ongoing" / "escalating" 四者之一，不许自创状态；
    ...
    messages = [
        {"role": "system", "content": LIFECYCLE_SYSTEM_PROMPT},
        {
            ...
            "content": (
                ...
                "再问「这件事了结了没有」；判的不是「这件事有多严重」，也不是「有多热」。"
            ),
        },
    ]
