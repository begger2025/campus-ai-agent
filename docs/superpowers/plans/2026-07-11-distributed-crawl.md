...
**接口契约（后续 Task 4 依赖，签名固定）：**
- `async claim_task(platform: str, worker_id: str) -> Optional[dict]` → `{"id": int, "keyword": str}` 或 None
- `async complete_task(task_id: int, status: str, items_stored: int, stop_reason: str) -> None`
- `async max_run_history_id(platform: str) -> int`
- `async run_history_delta(platform: str, before_id: int) -> tuple[int, str]`
...
def queue_env(monkeypatch):
    ...
    async def fake_complete(task_id, status, items_stored, stop_reason):
        completed.append((task_id, status, items_stored, stop_reason))

    async def fake_max_id(platform):
        return 100

    async def fake_delta(platform, before_id):
        return 5, "completed"
...
async def run_keyword_queue(crawler) -> None:
    ...
    while True:
        ...
        try:
            ...
            stored, reason = await crawl_queue.run_history_delta(platform, before_id)
            await crawl_queue.complete_task(task["id"], "done", stored, reason)
            utils.logger.info(
            ...
            await crawl_queue.complete_task(task["id"], "failed", 0, "exception")
            utils.logger.error(
