...
- 管线顺序（审查后修订）：strip → lower → 删 emoji（全串）→ 剥首尾标点 → 剥学校前缀 → **再剥一次首尾标点** → 既有判定（len<2/黑名单/纯数字）→ 限长。删表情/标点必须先于剥前缀，否则 "🔥中山大学食堂"/"#中山大学打卡#" 类标签的前缀剥不掉、黑名单可被绕过；剥前缀后需二次剥标点，防 "中山大学-食堂" 残留连字符。
- 贫瘠词 reason 文案定稿：有爬取时间 → "{N}天前爬过但无相关内容（已强降权）"；无爬取时间 → "近期爬过但无相关内容（已强降权）"。
